...
public class DeathController : Singleton<DeathController>
{
    ...
    [SerializeField] Button rollbackBtn;
    ...
    protected override void Awake()
    {
        ...
        if (deathUI == null)
        {
            Debug.LogError($"[DeathController] deathUI is null");
            return;
        }

        if (rollbackBtn == null)
        {
            Debug.LogError($"[DeathController] Rollback button is null");
            return;
        }
        else
        {
            rollbackBtn.AddButtonListener(OnRollbackButtonClick);
        }
    ...
    void OnEnable()
    {
        EventManager.Instance.Subscribe<OnPlayerDied>(ShowDeathUI);
    }

    void OnDisable()
    {
        EventManager.Instance.Unsubscribe<OnPlayerDied>(ShowDeathUI);
    }

    private void ShowDeathUI(OnPlayerDied _) => deathUI.SetActive(true);
    private void HideDeathUI() => deathUI.SetActive(false);
